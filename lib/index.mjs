  ...
  async function registerProviderRoute(providerName) {
    ...
    server.route({
      ...
      handler: async (request, h) => {
        console.log('Route', `/attach-${attr}-provider`, request.query)
        ...
        const isExist  = server.methods[options.methods.checkIfProviderIdentityAlreadyUsed](attr, { userId: id, email })
        if (isExist) {
          console.log('IS ALREADY LINKED')
          throw new Error('External account already attached on another account')
        }

        await server.methods[options.methods.attachProvider](decoded._id, attr, { userId: id, email })
